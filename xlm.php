...
<body>
  ...
  <div class="ui main container">
    <div class="ui secondary  menu">
      ...
      <a href="xlm.php" class="item active" id="priceLastmenuxlm">XLM</a>
...
$(document).ready(function () {
  ...
  function price(){
    ...
    $.getJSON('https://poloniex.com/public?command=returnTicker', function(data){
      document.getElementById('PoloniexLast').innerHTML = convertToRupiah(Math.floor(data.BTC_STR.last*btcPrice));
      document.getElementById('PoloniexBid').innerHTML = convertToRupiah(Math.floor(data.BTC_STR.highestBid*btcPrice));
      document.getElementById('PoloniexAsk').innerHTML = convertToRupiah(Math.floor(data.BTC_STR.lowestAsk*btcPrice));
    });

    $.getJSON('libs/bittrex.php?i=XLM', function(data){
    ...
    $.getJSON('https://vip.bitcoin.co.id/api/str_idr/ticker', function(data) {
      ...
      document.getElementById('totalVIP').innerHTML = Math.floor(data.ticker.vol_str);
      document.getElementById('totalVIPrp').innerHTML = convertToRupiah(Math.floor(data.ticker.vol_str)*data.ticker.high);
      document.title = "XLM " + convertToRupiah(data.ticker.last);
    });
  }
  ...
  function depth(){
    ...
    $.getJSON('https://vip.bitcoin.co.id/api/str_idr/trades', function(data){
      $.each(data, function(i,val){
        if(i > 5){
          ...
          document.getElementById('mp' + i).innerHTML = convertToRupiah(val.price*Math.floor(val.amount));
        });
      });

    $.getJSON('https://vip.bitcoin.co.id/api/str_idr/depth', function(data){  
